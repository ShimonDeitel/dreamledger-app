import XCTest
@testable import Dreamledger

final class DreamledgerTests: XCTestCase {
    var store: Store!
    ...
    @MainActor func testFreeLimitBlocksAdd() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(DreamEntry(), isPro: false)
        }
        let blocked = store.add(DreamEntry(), isPro: false)
    ...
    @MainActor func testProBypassesLimit() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(DreamEntry(), isPro: true)
        }
        let added = store.add(DreamEntry(), isPro: true)
        XCTAssertTrue(added)
    }
